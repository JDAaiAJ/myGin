...
func ClothingList(c *gin.Context) {
	var req struct {
		...
		Code     string `form:"code" json:"code" binding:"omitempty"` // 可选参数：按服装编码模糊查询
	}
	...
	db := config.DB.Model(&models.Clothing{})

	// 如果有 Code 参数，则添加模糊查询条件
	if req.Code != "" {
		db = db.Where("clothing.code LIKE ?", "%"+req.Code+"%")
	}

	// 联查 employee 表获取 name
	type ClothingWithUser struct {
		models.Clothing
	...
	var results []ClothingWithUser
	if err := db.Joins("JOIN user ON user.id = clothing.u_id").
		Select("clothing.*, user.name as e_name").
		Offset(offset).Limit(req.PageSize).Order("clothing.insert_time DESC").Find(&results).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"code":    500,
			"message": "查询失败",
		})
...
func AddClothing(c *gin.Context) {
	...
	var req struct {
		...
		Source string `form:"source" json:"source" binding:"omitempty"`
	...
	if err := config.DB.Create(&clothing).Error; err != nil {
		c.JSON(http.StatusOK, gin.H{"message": "保存服装信息失败", "code": 500})
		return
	}
...
func DeleteClothing(c *gin.Context) {
	...
	if err := config.DB.Where("id = ?", id).First(&clothing).Error; err != nil {
		c.JSON(http.StatusOK, gin.H{"message": "该服饰不存在", "code": 404})
		return
	}

	// 执行删除
	if err := config.DB.Delete(&clothing).Error; err != nil {
